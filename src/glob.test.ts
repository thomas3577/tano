...
Deno.test('computeHash > should return undefined when source is not provided', async () => {
  const actual = await computeHash();

  assertEquals(actual, undefined);
});

Deno.test('computeHash > should return undefined when no files are matched', async () => {
...
Deno.test('computeHash > should resolve a relative include against the root', async () => {
  const dir = await createTempDir();
...
Deno.test('computeHash > should cover nested directories', async () => {
  const dir = await createTempDir();
...
Deno.test('computeHash > should never hash .git, node_modules or .tano', async () => {
  const dir = await createTempDir();
...
Deno.test('computeHash > should change hash when an included file changes', async () => {
  const dir = await createTempDir();
...
Deno.test('computeHash > should change hash when the content changes but the size does not', async () => {
  const dir = await createTempDir();
...
Deno.test('computeHash > should hash the same sources equally regardless of where they are checked out', async () => {
  const hashes: string[] = [];
...
Deno.test('computeHash > should ignore excluded files', async () => {
  const dir = await createTempDir();
